...
#define __INTERNAL_DANGERFARM_CONTACT_IMPORT_protocol_database_sym(sym) \
    ...
    static inline int database_write_contact_form_append_request( \
    ...
    static inline int database_read_contact_form_append_request_payload( \
        DANGERFARM_CONTACT_SYM(contact_form)** x, int y) { \
            return  DANGERFARM_CONTACT_SYM( \
                        database_read_contact_form_append_request_payload)( \
                            x,y); \
    } \
    static inline int database_write_contact_form_append_response( \
        int x, const uint32_t y) { \
            return  DANGERFARM_CONTACT_SYM( \
                database_write_contact_form_append_response)(x,y); \
    } \
    static inline int database_read_contact_form_append_response( \
        uint32_t* x, int y) { \
            return DANGERFARM_CONTACT_SYM( \
    ...
    static inline int database_write_contact_form_get_count_response( \
        int x, const uint32_t y, const uint64_t z) { \
            return DANGERFARM_CONTACT_SYM( \
                        database_write_contact_form_get_count_response)( \
                            x,y,z); \
    } \
    static inline int database_read_contact_form_get_count_response( \
    ...
    static inline int database_read_contact_form_list_response( \
        uint32_t* w, uint64_t* x, uint64_t** y, int z) { \
        return DANGERFARM_CONTACT_SYM( \
            database_read_contact_form_list_response)(w,x,y,z); \
    } \
    static inline int database_write_contact_form_read_request( \
        int x, uint64_t y) { \
        return DANGERFARM_CONTACT_SYM( \
    ...
    static inline int database_write_contact_form_read_response( \
        int x, const uint32_t y, \
        const DANGERFARM_CONTACT_SYM(contact_form)* z) { \
            return DANGERFARM_CONTACT_SYM( \
                database_write_contact_form_read_response)(x,y,z); \
    } \
    static inline int database_read_contact_form_read_response( \
    ...
    static inline int database_read_contact_form_delete_request_payload( \
        uint64_t* x, int y) { \
            return DANGERFARM_CONTACT_SYM( \
                database_read_contact_form_delete_request_payload)(x,y); \
    } \
    static inline int database_write_contact_form_delete_response( \
        int x, const uint32_t y) { \
            return DANGERFARM_CONTACT_SYM( \
                database_write_contact_form_delete_response)(x,y); \
    } \
    static inline int database_read_contact_form_delete_response( \
        uint32_t* x, int y) { \
            return DANGERFARM_CONTACT_SYM( \
    ...
    static inline int database_read_database_truncate_response( \
        uint32_t* x, int y) { \
        return DANGERFARM_CONTACT_SYM( \
