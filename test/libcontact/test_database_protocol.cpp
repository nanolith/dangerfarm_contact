...
/**
 * We can read and write a contact form delete response.
 */
TEST(database_read_write_contact_form_delete_response)
{
    int sock[2];
    const uint32_t WRITE_STATUS = 2233;
    uint32_t status = 9999;
    ...
    /* write the delete response. */
    TEST_ASSERT(
        STATUS_SUCCESS
            == database_write_contact_form_delete_response(
                    sock[0], WRITE_STATUS));

    /* read the response payload. */
    TEST_ASSERT(
        STATUS_SUCCESS
            == database_read_contact_form_delete_response(
                    &status, sock[1]));
...
/**
 * We can write and read a database truncate request.
 */
TEST(database_read_write_database_truncate_request)
{
    int sock[2];
    uint32_t request_id = 1234;
    ...
    /* write the truncate request. */
    TEST_ASSERT(
        STATUS_SUCCESS
            == database_write_database_truncate_request(sock[0]));
    ...
    /* the request id matches. */
    TEST_ASSERT(DATABASE_REQUEST_ID_TRUNCATE == request_id);
...
/**
 * We can write a generic response and read it.
 */
TEST(database_read_write_generic_response)
{
    int sock[2];
    const uint32_t WRITE_STATUS = 2233;
    uint32_t status = 9999;
    ...
    TEST_ASSERT(
        STATUS_SUCCESS
            == database_write_generic_response(
                    sock[0], DATABASE_REQUEST_ID_TRUNCATE, WRITE_STATUS));
